...
--                           , lsSurface     :: S.Surface
--                           , lsCanvas      :: C.Surface
--                           , lsGermAnim    :: Time -> C.Render () -- FIXME: Move in GameState
--                           }
...
inputs required for the game:
- Time passed
- time since beginning
- events

mainLoop :: (GameInput -> GameState -> GameState) --

main :: IO ()
main = do
  backendState <- B.init "Epidemic" screenWidth screenHeight
  B.mainLoop backendState game

-}

mainSDL :: IO ()
mainSDL = do
  besRef <- B.initialize "Epidemic" screenWidth screenHeight (error "not defined gamestate")
